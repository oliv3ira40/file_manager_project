...
function webViewerInitialized() {
  ...
  // EU MUDEI AQUI EM BAIXO Ó
  file = 'file' in params ? window.location.origin+'/gerenciador/portaldosdirigentes/'+params.file : _app_options.AppOptions.get('defaultUrl');
...
function webViewerOpenFile() {
  // PERMISSÃO ABRIR NOVO ARQUIVO (CLICK)
  if (userHasThisPermission('Developer')) {
    var openFileInputName = PDFViewerApplication.appConfig.openFileInputName;
    document.getElementById(openFileInputName).click();
  }
}
function webViewerPrint() {
  // PERMISSÃO IMPRIMIR (CLICK)
  if (userHasThisPermission('admin.file_manager.download_file')) {
    window.print();
  }
}
...
function webViewerDocumentProperties() {
  // PERMISSÃO PROPRIEDADES DO ARQUIVO
  if (userHasThisPermission('Developer')) {
...
function webViewerKeyDown(evt) {
  ...
  if (cmd === 1 || cmd === 8) {
    switch (evt.keyCode) {
      case 83:
        // PERMISSÃO CRTL+S (DOWNLOAD DO ARQUIVO)
        if (userHasThisPermission('admin.file_manager.download_file')) {
          PDFViewerApplication.download();
        }
        handled = true;
...
var defaultOptions = {
  ...
  workerSrc: {
    value: 'node_modules/pdf_js/build/pdf.worker.js',
    kind: OptionKind.WORKER
  }
};
{
...
window.addEventListener('keydown', function (event) {
  if (event.keyCode === 80 && (event.ctrlKey || event.metaKey) && !event.altKey && (!event.shiftKey || window.chrome || window.opera)) {
    // PERMISSÃO CRTL+P (EVENTO DE IMPRESSÃO)
    if (userHasThisPermission('admin.file_manager.download_file')) {
      window.print();
    }
